...
import { getPermissionsPageList } from '#/api';
import {
  type PageInfo,
  type SysPermissions,
...
const queryData: SysPermissionsQuery = reactive({ NameOrCode: '' });
...
const tableData = reactive<SysPermissions[]>([]);
...
function LoadData() {
  ...
  getPermissionsPageList().then((p: SysPermissions[]) => {
...
const ruleForm = reactive<SysPermissions>(useDefaultSysPermissions());

const ruleFormRef = ref<FormInstance>();
const rules = reactive<FormRules<SysPermissions>>(sysPermissionsRules);
    ...
    <ElContainer style="height: 100%">
      <ElHeader style="line-height: 100px">
        <ElForm :inline="true" :model="queryData" class="demo-form-inline">
          <ElFormItem label="权限名/编号">
            <ElInput
              v-model="queryData.NameOrCode"
              clearable
              placeholder="请输入权限名或编号"
              style="width: 200px"
            />
          </ElFormItem>
          <ElFormItem>
